...
def _ensure_user(sess) -> tuple[int | None, str | None]:
    ...
    try:
        row = db.execute(
            text("INSERT INTO kanvas.chat_users (email) VALUES (:email) "
...
def _ensure_session(user_id, sid, first_message=None):
    ...
    try:
        if sid:
            ...
            if sid_int:
                row = db.execute(
                    text("SELECT id FROM kanvas.chat_sessions WHERE id = :sid AND user_id = :uid"),
        ...
        row = db.execute(
            text("INSERT INTO kanvas.chat_sessions (user_id, title) VALUES (:uid, :title) RETURNING id"),
...
def _list_sessions(user_id, limit=30):
    ...
    try:
        rows = db.execute(
            text("SELECT id, title, agent_slug, updated_at FROM kanvas.chat_sessions "
...
def _session_messages(session_id):
    ...
    try:
        rows = db.execute(
            text("SELECT role, content, agent_slug FROM kanvas.chat_messages "
...
def _persist_message(session_id, role, content, agent_slug=None, tool_calls=None):
    ...
    try:
        db.execute(
            text("INSERT INTO kanvas.chat_messages (session_id, role, content, agent_slug, tool_calls) "
        ...
        db.execute(
            text("UPDATE kanvas.chat_sessions SET updated_at = now() WHERE id = :sid"),
            {"sid": session_id},
        )
        db.commit()
    finally:
...
def register_chat_routes(rt):
    ...
    def app_home(sess, sid: str = ""):
        ...
        if uid and sid:
            try:
                ...
                try:
                    row = db.execute(
                        text("SELECT id, agent_slug FROM kanvas.chat_sessions WHERE id = :sid AND user_id = :uid"),
                        {"sid": int(sid), "uid": uid},
                    ).fetchone()
                    if row:
    ...
    async def chat_stream(request: Request):
        ...
        if not uid:
            ...
            try:
                row = db.execute(
                    ...
                    {"email": f"guest+{id(sess):x}@kanvas.local"},
                ).fetchone()
                db.commit()
                uid = row[0]
            finally:
        ...
        async def event_stream():
            ...
            db = _get_db()
            try:
                db.execute(text("UPDATE kanvas.chat_sessions SET agent_slug = :slug WHERE id = :sid"),
